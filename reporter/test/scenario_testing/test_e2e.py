...
def test_begin(generate_users, monkeypatch):
    ...
    assert total_distributed - int(claims_arv["aggregateRewards"]["amount"]) <= 1
    assert all(
    ...
    first issue: we're not correctly redistributing
    should be 100 eth redistributed because each staker is owed 100
    and one is inactive
    but this is wrong anyway: there should be 3 stakers
    ...
    assert total_distributed_prv - int(claims_prv["aggregateRewards"]["amount"]) <= 1
    assert all(
