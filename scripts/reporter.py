...
def write_participation(start_timestamp, voted):
    ...
    stakers = {
        web3.toChecksumAddress(addr): participation(addr, voters)
        for (addr, _) in get_stakers()
    }
...
def get_delegates():
    ...
    return [
        (web3.toChecksumAddress(d["delegator"]), web3.toChecksumAddress(d["delegate"]))
        for d in subgraph_delegates
    ]
...
def report_voters(start_timestamp, votes):
    ...
    voted = [
        {"address": addr} for addr in stakers_addrs_no_delegators if addr in voters
    ]
    voted.extend(
        [
            {"address": delegator}
            for (delegator, delegate) in delegates
            if delegate in voters
        ]
    )

    not_voted = [
        {"address": addr} for addr in stakers_addrs_no_delegators if addr not in voters
    ]
    not_voted.extend(
        [
            {"address": delegator}
...
def get_stakers_from_file(f):
    with open(f, "r") as stakers_file:
        ...
        for staker in reader:
            ...
            stakers.append(
                {"address": checksum_address, "amount": int(staker["amount"])}
            )
...
def write_reward_window(window_index, rewards):
    ...
    reward_window = {
        "chainId": 1,
        "rewardToken": SLICE_ADDRESS,
        "windowIndex": window_index,
        "totalRewardsDistributed": str(total_distributed),
    }
    recipients = {
        addr: {
            "amount": str(amount),
            "metaData": {"reason": [f"Distribution for epoch {window_index}"]},
        }
...
def compound_for_window(window_index, rewards, path_date):
    ...
    claimed = [
        web3.toChecksumAddress(item["account"])
        for item in get_claimed_for_window(prev_window)
    ]
    ...
    with open(prev_window_claims, "r") as f:
        ...
        claimed_prev_window = [
            {"address": web3.toChecksumAddress(address), "amount": item["amount"]}
            for (address, item) in claims_prev_window.items()
            if address in claimed
        ]
        claimed_prev_file = open(f"reports/staking/{path_date}/claimed.csv", "w+")
        claimed_writer = csv.DictWriter(
            claimed_prev_file, fieldnames=["address", "amount"]
        )
        ...
        for (
            claim_addr,
            claim_obj,
        ) in claims_prev_window.items():  # get unclaimed claims from previous epoch
            if web3.toChecksumAddress(claim_addr) not in claimed:  # unclaimed
                unclaimed.append(
                    {"address": claim_addr, "amount": int(claim_obj["amount"])}
                )
    ...
    compound_report = {
        ...
        "total_addresses_unclaimed": len(unclaimed),
    }
...
def inactive_for_window(window_index, rewards, path):
    ...
    with open(f"{path}/not_voted.csv") as non_voters_file:
        ...
        inactive_addresses = [
            item["address"] for item in list(reader)[1:]
        ]  # take non-voting addresses
        prev_inactive_addresses = [
            item["address"] for item in prev_inactive
        ]  # take prev window non-voting addresses
        prev_inactive = {
            item["address"]: {
                "amount": item["amount"],
                "inactive_windows": item["inactive_windows"],
            }
            for item in prev_inactive
        }
            ...
            if addr in inactive_addresses:  # if in currently non-voters addresses:
                if addr in prev_inactive_addresses:  # and non-voter previously:
                    inactive_item = {
                        "address": addr,
                        "amount": amount,
                        "inactive_windows": prev_inactive[addr]["inactive_windows"]
                        + [window_index],
                    }
                else:
                    inactive_item = {
...
def report_prorata(path, path_date):
    ...
    stakers = [
        (stk["address"], stk["amount"])
        for stk in get_stakers_from_file(f"{path}/stakers.csv")
    ]
    ...
    rewards_serialized = [
        {"address": addr, "amount": amt} for (addr, amt) in rewards.items()
    ]
    ...
    (rewards, unclaimed, compound_report) = compound_for_window(
        window_index, rewards, path_date
    )  # compound rewards
    ...
    write_csv(
        rewards_serialized,
        f"{path}/slice_amounts_after_unclaimed.csv",
        ["address", "amount"],
    )

    (new_rewards, inactive) = inactive_for_window(
        window_index, rewards, path=path
    )  # account inactive stakers
...
def report():
    ...
    write_staking_csv(
        int(start_date.timestamp()), stakers, "stakers.csv", stakers_fieldnames
    )

    report_prorata(
        f"reports/staking/{date.year}-{date.month}", f"{date.year}-{date.month}"
    )
